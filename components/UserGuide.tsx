...
export const UserGuide: React.FC<Props> = ({ isHe, onClose }) => {
  ...
  const steps = [
    ...
    },
    {
      ...
      visualEmoji: <Zap size={64} className="text-yellow-400" />,
      color: "bg-emerald-50",
      content: (
    ...
    },
    {
      ...
      description: isHe
        ? "בחרו 'סיור חכם' כדי לגלות הכל סביבכם, או 'מסלול רחוב' לטיול ממוקד לאורך רחוב."
        : "Choose 'Smart Tour' to discover surroundings, or 'Street Tour' for a focused walk along a street.",
      visualEmoji: "🚥",
      color: "bg-amber-50",
      content: (
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6">
      ...
      >
        <div className={`flex-1 flex flex-col items-center justify-center p-10 text-center transition-colors duration-700 ${current.color}`}>
          {current.visualEmoji && (
            <div className="mb-6 transform transition-transform duration-700 hover:scale-110 drop-shadow-lg text-7xl">
              {current.visualEmoji}
            </div>
          )}
